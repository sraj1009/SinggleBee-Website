...
const QuickViewModal: React.FC<QuickViewModalProps> = ({ product, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-end sm:items-center justify-center p-0 sm:p-4 md:p-12 animate-fade-in">
      <div
        className="absolute inset-0 bg-brand-black/40 backdrop-blur-xl"
        onClick={onClose}
      />

      <div className="relative bg-white/90 backdrop-blur-md rounded-t-2xl sm:rounded-[2rem] md:rounded-[3.5rem] shadow-[0_30px_100px_rgba(0,0,0,0.3)] overflow-hidden w-full max-w-5xl max-h-[90vh] sm:max-h-[90vh] flex flex-col md:flex-row animate-slide-up border-t-2 sm:border border-white/50">
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-3 right-3 sm:top-6 sm:right-6 md:top-8 md:right-8 z-50 w-9 h-9 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-xl sm:rounded-2xl bg-white shadow-xl flex items-center justify-center text-brand-black hover:bg-brand-primary transition-all active:scale-90 font-black text-sm"
        >
          ✕
        </button>

        {/* Left Column: Image Centerpiece */}
        <div className="md:w-1/2 p-4 sm:p-8 md:p-16 flex items-center justify-center bg-gradient-to-br from-amber-50/80 to-orange-50/50 border-b md:border-b-0 md:border-r border-white/50 relative overflow-hidden min-h-[200px] sm:min-h-[280px]">
        ...
        <div className="md:w-1/2 p-5 sm:p-8 md:p-16 flex flex-col overflow-y-auto no-scrollbar">
          <div className="mb-auto">
            ...
            {/* Title & Author */}
            <h2 className="text-2xl sm:text-3xl lg:text-5xl font-black text-brand-black tracking-tighter leading-tight mb-2 sm:mb-4">
              {product.title}
            </h2>
            <p className="text-gray-400 font-bold text-sm sm:text-base md:text-xl mb-4 sm:mb-6 md:mb-10 italic">
            ...
            {/* Price */}
            <div className="flex flex-wrap items-baseline gap-2 mb-4 sm:mb-6 md:mb-10">
              <span className="text-3xl sm:text-4xl md:text-5xl font-black text-brand-black tracking-tighter">
                ₹{product.price.toLocaleString('en-IN')}
              </span>
              <span className="text-gray-400 text-[10px] sm:text-xs md:text-sm font-bold uppercase tracking-widest bg-gray-100 px-2 py-0.5 rounded">Inclusive of all taxes</span>
            </div>

            {/* Description Snapshot */}
            <div className="space-y-2 sm:space-y-3 md:space-y-4 mb-6 sm:mb-8 md:mb-12">
              <h4 className="text-[9px] sm:text-[10px] uppercase tracking-[0.2em] font-black text-gray-400">The Hive Snapshot</h4>
              <p className="text-gray-600 font-medium leading-relaxed text-sm sm:text-base md:text-lg line-clamp-3 sm:line-clamp-4">
                {product.description}
              </p>
            </div>
          </div>
          ...
          {/* Action Footer */}
          <div className="pt-4 sm:pt-6 md:pt-10 border-t border-brand-primary/10 flex flex-col gap-3 sm:gap-4">
            <button
